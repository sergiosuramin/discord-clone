  ...
  const onSubmit = async (values: TChatInputSchema) => {
    try {
      ...
      if (error instanceof AxiosError) {
        toast.error(error?.response?.data ?? 'Failed to post message')
      }
    }
  }
    ...
    <div className="tw-relative tw-group tw-flex tw-items-center hover:tw-bg-black/10 tw-p-4 tw-transition tw-w-full">
      <div className="tw-group tw-flex tw-gap-x-2 tw-items-start tw-w-full">
        ...
        <div className="tw-flex tw-flex-col tw-w-full">
          ...
          {/* generalized every deleted message */}
          {deleted ? (
            <p className="tw-text-zinc-500 dark:tw-text-zinc-400 tw-italic tw-opacity-70 dark:tw-opacity-50 tw-text-xs tw-mt-1">
            ...
            <>
              ...
              {!fileUrl && !isEditing && (
                <p className="tw-text-sm tw-text-zinc-600 dark:tw-text-zinc-300">
                  {content}
                  {isUpdated && (
